let AES = {
    ...
    cipher: function (input, w) {
        ...
        this.subBytes(state, nb);
        this.shiftRows(state, nb);
        this.addRoundKey(state, w, 10, nb);
    ...
    invShiftRows: function (state, nb) {
        let temp = new Array(nb);
        for (let r = 1; r < 4; r++) {
            for (let c = 0; c < nb; c++) {
            ...
            for (let c = 0; c < nb; c++) {
                state[r][c] = temp[(c + (nb - r)) % nb];
            }
        }
    },

    invMixColumns: function (state, nb) {
        let r, c, a, b;
        let mult = [0x0e, 0x0b, 0x0d, 0x09];
        ...
        for (c = 0; c < nb; c++) {
            ...
            for (r = 0; r < 4; r++) {
                state[r][c] =
                    this.multGF(a[r], mult[0]) ^
                    this.multGF(a[(r + 3) % 4], mult[1]) ^
                    this.multGF(a[(r + 2) % 4], mult[2]) ^
                    this.multGF(a[(r + 1) % 4], mult[3]);
            }
        }
    },
